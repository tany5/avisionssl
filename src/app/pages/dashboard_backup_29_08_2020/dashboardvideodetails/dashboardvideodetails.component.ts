...
files: FileList
  ...
  constructor(private videoService:DashboardvideodetailsService , private route: ActivatedRoute , private router: Router) {
     this.videoService.getProductId(this.route.snapshot.params.prodId).subscribe(
       (res) => {
        this.prodId = res['product_id']
        this.videoService.getCourseDetails(this.prodId).subscribe(
          (res)=> {       
          ...
          this.videoService.getDemoChapterVideo(this.prodId).subscribe(
          (res) => {
            console.log("demo...");
            console.log(res)
          ...
          this.videoService.getVieoCourseSubject(this.prodId).subscribe(
            (res) => {    
             this.subjectLoader = true      
              this.videoDetailsSubData = res['vdo_details_sub_data']
              this.videoDetailsSubDataLoader = true
              this.videoService.getVideoCourseChapter(this.prodId,this.videoDetailsSubData[0].subject_id).subscribe(
               (data)=> {
           ...
           this.videoService.geVideoCourseTeacherDetails(this.prodId).subscribe(
             (res)=> {        
  ...
  ngOnInit(): void {
    ...
    this.videoService.getProductId(this.route.snapshot.params.prodId).subscribe(
      (res) => {
        this.prodId = res['product_id']
        this.videoService.getbuystat(localStorage.getItem('currentUserId'),this.prodId).subscribe(
          (data) => {
            if(data['buy_stat'] == 1){
              this.buystat = 1;
  ...
  fetchChapter(subject_id){
      ...
      this.videoService.getVideoCourseChapter(this.prodId,subject_id).subscribe(
        (res)=> {
          this.chapterLoader = true
    ...
    indicateVdocourse(){
      ...
      jQuery("#product_id").val(this.prodId);
        ...
        this.videoService.getbuystat(localStorage.getItem('currentUserId'),this.prodId).subscribe(
          (res) => {
            if(res['buy_stat'] == 1){
              ...
              this.buystat = 0;
              location.href='https://avision.co.in/adminpanel/index.php/api/pay_now/'+localStorage.getItem('currentUserId')+'/'+this.prodId;
            }
          }
        )
    ...
    showDemoVideos(vdo_id){
      ...
      if(localStorage.getItem('currentUser') == '1'){
        this.router.navigateByUrl('show-video/'+vdo_id+'/'+this.prodId)
      }else{
