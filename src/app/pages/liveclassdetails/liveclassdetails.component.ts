...
import { LiveclassdetailsService } from './liveclassdetails.service';
...
@Component({
  selector: 'app-liveclassdetails',
  templateUrl: './liveclassdetails.component.html',
  styleUrls: ['./liveclassdetails.component.scss']
})
export class LiveclassdetailsComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,private router:Router, private liveService: LiveclassdetailsService,private EncrDecr: EncrDecrServiceService,private metatagservice: MetatagServiceService,private titleService: Title,private metaTagService: Meta) {   

    //this.prodId = this.route.snapshot.params.prodId
    this.liveService.getProductId(this.route.snapshot.params.prodId).subscribe(

      (res) => {
        this.prodId = res['product_id']
        
        this.encryptProdId = this.EncrDecr.set('123456$#@$^@1ERF', this.prodId)
        this.liveService.geLiveClassDetails(this.prodId).subscribe(
          (res)=> {
         ...
         this.liveService.geLiveClassTeacherDetails(this.prodId).subscribe(
           (res)=> {        
          ...
          this.liveService.gettestimonialDetails().subscribe(
            (res)=> {
              console.log(res)          
