...
@Component({
  selector: 'app-mainfreevideos',
  templateUrl: './mainfreevideos.component.html',
  styleUrls: ['./mainfreevideos.component.scss']
})
export class MainfreevideosComponent implements OnInit {
  href: any
  ...
  constructor(private router: Router, private location: Location,private route:ActivatedRoute, private webServie: WebserviceService) { 
    ...
    console.log(this.mainUrl , this.mainUrl3)
    ...
    this.webServie.api(this.mainUrl,headers).subscribe(
      (res)=> {
        ...
        console.log(this.searchVideoLists[0].snippet['thumbnails']['medium']['url'])
      })
  ...
  videoDetails(videoId, searchQuery) { 
    ...
    this.router.navigate([`freeliveclassvideo/${videoId}/${this.searchQuery}`])   
  }
