...
export class PracticetestComponent implements OnInit {
  ...
  async getChapter(subject_id){
    let chapter_list =[]
    this.count_arr = await this.practiceService.countAppliedQuestion(this.user_id,subject_id).toPromise()
    await this.practiceService.get_practice_chapter(subject_id).subscribe(

      (data) => {
        this.chapters = data['chap_arr']
    ...
    /*this.practiceService.get_practice_chapter(subject_id,this.user_id).subscribe(
      (res) => {
        var i_count;
        for(var i=0; i< res['chap_arr'].length; i++){
          i_count=i
          let chapter_id = res['chap_arr'][i].chapter_id
          this.practiceService.countAppliedQuestion(this.user_id,subject_id,res['chap_arr'][i].chapter_id).subscribe(
            (data) => {
              //console.log(data);
              //console.log(chapter_id)
              this.count_arr.push({'chapter_id':chapter_id,'applied_count':data['count']})


              if((i_count+1) == res['chap_arr'].length){

                this.chapters = res['chap_arr']
                this.final_count = this.count_arr
  ...
  loadcount(subject_id,chapter_id){
