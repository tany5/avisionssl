...
export class ResumeComponent implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute, private exampanelService: ResumeService, private formbuilder: FormBuilder, private modalService: NgbModal, private elRef: ElementRef, private location: LocationStrategy) {
    ...
    this.exampanelService.getQuizInformation(this.prodId,localStorage.getItem("currentUserId"),localStorage.getItem("test_taken_id")).subscribe(
      (res) => {
        this.quiz_info = res['quiz_information']['quiz_info']
        this.quiz_details = res['quiz_information']['quiz_details']
        this.quizTabLists = res['quiz_information']['quiz_tab_list']

        console.log(this.quiz_details)

        this.quizTabLists = this.quizTabLists[0]
        ...
        if(this.quiz_details[0]['left_time_status']==1) {          
          this.duration1 = this.quiz_details[0]['left_time']
          this.time = this.duration1.split(':')
          this.duration = parseInt(this.time[0])*3600 +  parseInt(this.time[1])*60 +  parseInt(this.time[2])
          this.customFormat = { leftTime: this.duration }         
        }
    ...
    if (this.changeable == 0) {
      if (event.action == "done") {
        if (this.quiz_details.length == this.count) {
          ...
          this.selected = this.quiz_details[this.count]['question_type_id']
